...
def zoneData():
    ...
    for i in range(len(zonesD)):
        ...
        coordinates=matcher_detailed(dist.lower(),pradesh.lower(),latLong,district=True)
        if(coordinates!=0):
    ...
    zonesPanda['lat']=Zlat
    zonesPanda.to_excel('zones.xlsx')
    return zonesPanda

def matcher_final(pdat_i,pdats_i,zonesPanda,district=True):
    key=999999999
    city=False
...
def jsonWriter(dist,state,conf,act,rec,dead,coor,zone,stateCounter,india_totalCases,india_activecases,india_recovered,india_deaths):
    if(coor):
...
def matcher_detailed(pdat_i,pdats_i,latLong,district=True):
    key=999999999
    city=False
...
def coronavirus():
    mainer(False)
    return "COVID19 India data updated!"

@app.route("/updator",methods=["GET"])
def coronaviruszones():
    mainer(True)
    return "COVID19 India data updated along with the zones from covid19india.org!"
